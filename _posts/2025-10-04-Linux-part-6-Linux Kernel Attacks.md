...
حالا که یکم دستمون توی لینوکس راه افتاده، توی این بخش میخوایم باهم یه rootkit ساده در لینوکس توسعه بدیم و بریم ببینیم مهاجمین چجوری و با چه هدفی از kernel module ها استفاده میکنند. نکته مهم همینه شما در لینوکس زمانی که دسترسی root دارید دیگه دستتون خیلی بازه برای خیلی کارها از جمله اینکه یه ماژول کرنل توسعه و نصب کنید. قبلش باید یه سری مفاهیم رو هرچند سطحی اینجا بهش بپردازیم و بعدا احتمالا به این ها برمیگردیم و بیشتر درباه شون حرف میزنیم.
...
خب حالا برای اینکه درک کنید این که شما یه کرنل ماژول در سیستمون نصب بشه چقدر میتونه بهتون آسیب بزنه میخوایم یه kernel module بهتون نشون بدیم که کارش keylog کردن هست:) بعد اونو از لیست lsmod حذف کنیم البته ما وقتی root هستیم در لینوکس شبیه خداییم فقط میگم "کن فیکون" ما میتونیم اینجا حتی از لیست پراسس ها حذف کنیم برنامه رو یا مثلا روی یه پورت listen کنیم ولی اون رو hide کنیم از خروجی که netstat نشون میده و هزار کار اینجوری :)
...
شما این رو در نظر بگیر که مهاجم دیگه وقتی کلید رو به دست میاره در dmesg لاگ نمیکند بلکه آن را به c2 server خود ارسال میکند.
